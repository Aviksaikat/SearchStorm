...
def highlight_pattern(text, pattern):
	...
	new_text = text[:start] + style(highlighted, fg='blue') + text[start+len(pattern):]
	#print(new_text)
	return new_text
...
def print_result(result, pattern):
	...
	data = [[ match[3], match[0], highlight_pattern(match[1], pattern), match[2]] for match in result]
	#print(data)
	
	table = columnar(data, headers,  patterns=patterns, justify="c", wrap_max=5, max_column_width=None)
	print(table)
...
def print_lines(result, directory):
	for i in range(len(result)):
		...
		#print(file_name)
		remote_url = subprocess.run(["git", "-C", directory, "remote", "get-url", "origin"], stdout=subprocess.PIPE, text=True).stdout.replace('\n', '').replace(".git", '')
		print(str(remote_url) + '/' + "blob/main/" + str(file_name) + '#L' + str(line_number))

if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Search for pattern in files recursively")
	...
	parser.add_argument("-e", "--extension", help="File extension to search (default: all files)")
